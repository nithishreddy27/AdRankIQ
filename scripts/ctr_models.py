...
class CTRXGBoostModel:
    ...
    def train(self, X_train, y_train, X_val, y_val, num_rounds=1000, early_stopping_rounds=50):
        """Train XGBoost model for CTR prediction"""
        print("Training XGBoost CTR model...")
        ...
        print("XGBoost CTR model training completed!")
    ...
    def evaluate(self, X_test, y_test):
        ...
        print(f"XGBoost CTR Model Performance:")
...
class CTRNeuralNetworkTrainer:
    def __init__(self, input_dim, hidden_dims=[512, 256, 128, 64], dropout_rate=0.3, learning_rate=0.001):
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        print(f"Using device: {self.device}")
        
        self.model = CTRNeuralNetwork(input_dim, hidden_dims, dropout_rate).to(self.device)
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100, batch_size=1024, early_stopping_patience=10):
        """Train neural network for CTR prediction"""
        print("Training Neural Network CTR model...")
        ...
        for epoch in range(epochs):
            ...
            if val_auc > best_val_auc:
                ...
                patience_counter = 0
                # Save best model
                torch.save(self.model.state_dict(), 'best_ctr_nn_model.pth')
            else:
                patience_counter += 1
        ...
        # Load best model
        self.model.load_state_dict(torch.load('best_ctr_nn_model.pth'))
        print(f"Neural Network CTR model training completed! Best Val AUC: {best_val_auc:.4f}")
    ...
    def evaluate(self, X_test, y_test):
        ...
        print(f"Neural Network CTR Model Performance:")
        print(f"AUC: {auc_score:.4f}")
        print(f"Log Loss: {logloss:.4f}")
        print(f"PR-AUC: {pr_auc:.4f}")
    ...
    def plot_training_history(self):
        ...
        plt.tight_layout()
        plt.savefig('ctr_training_history.png', dpi=300, bbox_inches='tight')
        plt.show()

def load_and_prepare_data(sample_size=1000000):
    """Load and prepare data for CTR model training"""
    print(f"Loading and preparing {sample_size:,} samples for CTR training...")
    
    # For demonstration, we'll create sample data
    # In practice, this would load from the generated dataset chunks
    np.random.seed(42)
    ...
    # Generate realistic CTR labels
    ctr_logits = (
        -2.5 +
        0.3 * (data['item_rating'] - 3) +
        0.2 * np.log(data['item_review_count'] + 1) +
        -0.1 * np.log(data['item_price']) +
        0.2 * (data['user_previous_purchases'] > 0) +
        0.1 * data['is_weekend'] +
    ...
    print(f"Data loaded. CTR: {data['click'].mean():.4f}")
    return data

def train_ctr_models():
    """Main function to train both XGBoost and Neural Network CTR models"""
    print("="*60)
    print("TRAINING CTR PREDICTION MODELS")
    print("="*60)
    
    # Load and prepare data
    data = load_and_prepare_data(1000000)  # 1M samples for demo
    
    # Feature engineering
    fe = FeatureEngineer()
    engineered_data = fe.engineer_features(data)
    
    # Get feature matrix for CTR
    X, y = fe.get_feature_matrix(engineered_data, 'ctr')
    ...
    xgb_model = CTRXGBoostModel()
    xgb_model.train(X_train, y_train, X_val, y_val)
    xgb_metrics = xgb_model.evaluate(X_test, y_test)
    xgb_model.save_model('ctr_xgboost_model.model')
    ...
    nn_trainer = CTRNeuralNetworkTrainer(
    ...
    best_auc = nn_trainer.train(X_train, y_train, X_val, y_val, epochs=100, batch_size=1024)
    ...
    results = {
        ...
        'target_auc_achieved': max(xgb_metrics['auc'], nn_metrics['auc']) >= 0.91
    }
    
    with open('ctr_model_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save feature engineering config
    fe.save_feature_config('ctr_feature_config.json')
    
    print("\n" + "="*60)
    print("CTR MODEL TRAINING RESULTS")
    print("="*60)
    print(f"XGBoost AUC: {xgb_metrics['auc']:.4f}")
    print(f"Neural Network AUC: {nn_metrics['auc']:.4f}")
    print(f"Target AUC (0.91) achieved: {results['target_auc_achieved']}")
    print(f"Best model: {'XGBoost' if xgb_metrics['auc'] > nn_metrics['auc'] else 'Neural Network'}")
    
    return results

if __name__ == "__main__":
    results = train_ctr_models()
